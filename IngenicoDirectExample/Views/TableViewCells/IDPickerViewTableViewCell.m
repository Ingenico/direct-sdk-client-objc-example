...
@interface IDPickerViewTableViewCell ()

@property (strong, nonatomic) IDPickerView *pickerView;

@end

@implementation IDPickerViewTableViewCell
...
- (void)setItems:(NSArray<IDValueMappingItem *> *)items {
    _items = items;
    if (items != nil) {
        NSMutableArray *names = [[NSMutableArray alloc]initWithCapacity:items.count];
        for (IDValueMappingItem *item in items) {
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    self.pickerView = [IDPickerView new];
...
- (void)layoutSubviews {
    ...
    if (self.pickerView != nil) {
        ...
        CGFloat height =  [IDPickerViewTableViewCell pickerHeight];
