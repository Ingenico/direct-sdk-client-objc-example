...
@interface IDPaymentProductsViewControllerTarget ()
...
@property (strong, nonatomic) IDPaymentProduct *applePayPaymentProduct;
...
@implementation IDPaymentProductsViewControllerTarget
...
- (id)init {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"-init is not a valid initializer for the class IDPaymentProductsViewControllerTarget"
                                 userInfo:nil];
    return nil;
}
...
- (void)didSelectPaymentItem:(NSObject <IDBasicPaymentItem> *)paymentItem accountOnFile:(IDAccountOnFile *)accountOnFile; {
    ...
    // payment product in the payment product selection screen, the IDSession
    ...
    [self.session paymentProductWithId:paymentItem.identifier context:self.context success:^(IDPaymentProduct *paymentProduct) {
        if ([paymentItem.identifier isEqualToString:kIDApplePayIdentifier]) {
            ...
            if (paymentProduct.fields.paymentProductFields.count > 0) {
                ...
                IDPaymentRequest *request = [[IDPaymentRequest alloc] init];
        ...
        [SVProgressHUD dismiss];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"ConnectionErrorTitle", kIDAppLocalizable, @"Title of the connection error dialog.") message:NSLocalizedStringFromTable(@"PaymentProductErrorExplanation", kIDAppLocalizable, nil) delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
    }];
}

- (void)showPaymentItem:(NSObject <IDPaymentItem> *)paymentItem accountOnFile:(IDAccountOnFile *)accountOnFile {
    IDPaymentProductViewController *paymentProductForm = nil;
    
    if (([paymentItem isKindOfClass:[IDPaymentProductGroup class]] && [paymentItem.identifier isEqualToString:@"cards"]) || ([paymentItem isKindOfClass:[IDPaymentProduct class]] && [((IDPaymentProduct *)paymentItem).paymentMethod isEqualToString:@"card"])) {
        paymentProductForm = [[IDCardProductViewController alloc] init];

    }
    else {
        paymentProductForm = [[IDPaymentProductViewController alloc] init];
    }
...
- (void)showApplePayPaymentItem:(IDPaymentProduct *)paymentProduct {
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0") && [PKPaymentAuthorizationViewController canMakePayments]) {
        ...
        [self.session paymentProductNetworksForProductId:kIDApplePayIdentifier context:self.context success:^(IDPaymentProductNetworks *paymentProductNetworks) {
            [self showApplePaySheetForPaymentProduct:paymentProduct withAvailableNetworks:paymentProductNetworks];
            [SVProgressHUD dismiss];
        } failure:^(NSError *error) {
            [SVProgressHUD dismiss];
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"ConnectionErrorTitle", kIDAppLocalizable, @"Title of the connection error dialog.") message:NSLocalizedStringFromTable(@"PaymentProductNetworksErrorExplanation", kIDAppLocalizable, nil) delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alert show];
        }];
    }
}
...
- (void)showApplePaySheetForPaymentProduct:(IDPaymentProduct *)paymentProduct withAvailableNetworks:(IDPaymentProductNetworks *)paymentProductNetworks {
    ...
    NSString *merchantId = [StandardUserDefaults objectForKey:kIDMerchantId];
    if (merchantId == nil) {
        return;
    }
...
- (void)generateSummaryItems {
    ...
    [summaryItems addObject:[PKPaymentSummaryItem summaryItemWithLabel:NSLocalizedStringFromTableInBundle(@"gc.app.general.shoppingCart.subtotal", kIDSDKLocalizable, self.sdkBundle, @"subtotal summary item title") amount:[NSDecimalNumber decimalNumberWithMantissa:subtotal exponent:-2 isNegative:NO]]];
    [summaryItems addObject:[PKPaymentSummaryItem summaryItemWithLabel:NSLocalizedStringFromTableInBundle(@"gc.app.general.shoppingCart.shippingCost", kIDSDKLocalizable, self.sdkBundle, @"shipping cost summary item title") amount:[NSDecimalNumber decimalNumberWithMantissa:shippingCost exponent:-2 isNegative:NO]]];
...
- (void)didSubmitPaymentRequest:(IDPaymentRequest *)paymentRequest {
    [self didSubmitPaymentRequest:paymentRequest success:nil failure:nil];
}

- (void)didSubmitPaymentRequest:(IDPaymentRequest *)paymentRequest success:(void (^)(void))succes failure:(void (^)(void))failure {
    [SVProgressHUD showWithStatus:NSLocalizedStringFromTableInBundle(@"gc.app.general.loading.body", kIDSDKLocalizable, [NSBundle bundleWithPath:kIDSDKBundlePath], nil)];
    [self.session preparePaymentRequest:paymentRequest success:^(IDPreparedPaymentRequest *preparedPaymentRequest) {
        [SVProgressHUD dismiss];
        ...
        [SVProgressHUD dismiss];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"ConnectionErrorTitle", kIDAppLocalizable, @"Title of the connection error dialog.") message:NSLocalizedStringFromTable(@"SubmitErrorExplanation", kIDAppLocalizable, nil) delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
        if (failure != nil) {
            failure();
        }
